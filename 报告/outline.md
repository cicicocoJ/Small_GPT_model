...
\section{从零构建大语言模型的关键知识点梳理}

\subsection{从零构建大模型的总体流程}

\subsubsection{大模型构建的基本环节}

说明从零构建大模型通常包括：训练语料准备、文本清洗与分词、训练样本构造、模型架构设计、模型参数配置、预训练、模型评估、微调与任务适配、推理生成等环节。强调这些环节不是孤立的，而是相互影响的。例如，分词方式会影响词表大小，词表大小会影响嵌入层和输出层参数量；上下文长度会影响训练样本构造，也会显著影响注意力计算开销。

\subsubsection{本项目的小型化实现思路}

说明由于本项目在普通单机和低算力条件下完成实验，因此并不是复现真正的大规模 GPT，而是保留 GPT 类语言模型的核心机制，包括 Decoder-only 结构、自回归建模、因果自注意力、next-token prediction 预训练和监督微调等，再通过减小层数、嵌入维度、上下文长度和训练数据规模来降低计算成本。

建议加入一张“从零构建 MiniGPT 流程图”：
原始文本数据 $\rightarrow$ 文本清洗 $\rightarrow$ 分词与词表构建 $\rightarrow$ token 序列 $\rightarrow$ 滑动窗口构造训练样本 $\rightarrow$ Decoder-only GPT 模型 $\rightarrow$ 预训练 $\rightarrow$ 生成测试 $\rightarrow$ 分类任务微调。

\subsection{语言模型的基本任务与自回归建模}

\subsubsection{语言模型的目标}

说明语言模型的核心任务是对文本序列的概率分布进行建模，即根据已有上下文预测后续内容。可以保留你现有的公式：
[
P(x_1,x_2,\cdots,x_T)=\prod_{t=1}^{T}P(x_t|x_{<t})
]
并解释它表示模型将整段文本的生成过程拆解为多个“预测下一个 token”的子问题。

\subsubsection{Next-token prediction 训练目标}

说明 GPT 类模型在预训练阶段不需要人工标签，而是直接从连续文本中自动构造输入和目标。输入序列为 $x_1,x_2,\cdots,x_{T-1}$，目标序列为右移一位后的 $x_2,x_3,\cdots,x_T$。模型在每个位置预测下一个 token，并使用交叉熵损失计算预测分布与真实 token 之间的差异。

\subsubsection{自回归生成机制}

说明预训练完成后，模型可以根据 prompt 逐步生成文本。每一步输出词表上所有 token 的 logits，再通过贪心搜索、温度采样、top-k 或 top-p 等方法选择下一个 token，并将其拼接回上下文继续生成。这里不需要展开实验结果，只需要说明生成过程的知识点。

\subsection{训练数据准备与文本表示}

\subsubsection{训练语料的选择}

说明大模型训练需要大量高质量文本数据，语料的规模、领域、语言风格和噪声水平都会影响模型效果。对于低算力 MiniGPT，语料不宜过大、过复杂，否则训练时间长且模型难以收敛。因此本项目选择 TinyStories 这类短篇英文故事数据集，原因是文本短、语言简单、句式清晰、情节完整，适合小模型学习基础语言规律。

\subsubsection{文本清洗与拼接}

说明原始文本通常需要进行基础清洗，例如去除空文本、异常符号、重复数据或过短样本。对于多个短故事样本，可以将其拼接成较长的训练文本，并在不同故事之间加入特殊分隔符。这里可以把 \texttt{<|endoftext|>} 表述为“文本结束标记”或“样本边界标记”，说明它用于提示模型不同故事之间的边界，避免模型把两个无关故事误认为同一段连续叙事。

\subsubsection{分词与词表构建}

说明神经网络不能直接处理字符串，需要先通过 tokenizer 将文本转换为 token 序列。比较字符级分词、词级分词、正则分词和 BPE 子词分词的特点：

字符级分词实现简单、词表小，但生成结果容易碎片化；词级分词可读性强，但遇到未登录词时处理困难；BPE 子词分词泛化能力较强，是 GPT 类模型常用方案，但词表可能较大；正则分词可以在实现难度、词表大小和生成可读性之间取得折中。

\subsubsection{Token id、词嵌入与位置嵌入}

说明分词后，每个 token 会被映射为词表中的编号，即 token id。模型首先通过词嵌入矩阵将离散 token id 转换为连续向量，再加入位置嵌入，使模型能够感知 token 在序列中的顺序。可以保留你现有的公式：
[
\mathbf{h}*0=\mathbf{E}*{tok}(\mathbf{x})+\mathbf{E}_{pos}(\mathbf{p})
]

\subsection{GPT 模型架构与关键模块}

\subsubsection{Transformer 与 Decoder-only 结构}

说明 Transformer 是 GPT 的基础结构，而 GPT 采用的是 Decoder-only 架构。与 Encoder-only 模型更适合理解任务不同，Decoder-only 模型通过因果遮罩实现从左到右生成，因此更适合文本续写和开放式生成任务。

\subsubsection{因果自注意力机制}

说明自注意力机制通过 Query、Key、Value 计算不同 token 之间的相关性，使模型能够根据上下文选择重要信息。GPT 中需要加入因果遮罩，使当前位置不能关注未来位置，从而保证训练目标与推理生成方式一致。可以保留你现有的注意力公式。

\subsubsection{多头注意力机制}

说明多头注意力是在多个子空间中并行计算注意力，不同注意力头可以关注不同类型的上下文关系，例如局部词语搭配、句法结构、人物关系或前后呼应。最后将多个头的输出拼接并线性变换，形成当前层的上下文表示。

\subsubsection{前馈网络、残差连接与 LayerNorm}

说明 Transformer block 中除了注意力层，还包括前馈神经网络。注意力层主要负责上下文信息交互，前馈网络负责对每个位置的表示进行非线性变换。残差连接有助于缓解深层网络训练困难，LayerNorm 有助于稳定激活分布和训练过程。

\subsubsection{输出层与 logits}

说明经过多层 Transformer block 后，模型会得到每个位置的隐藏状态。输出层将隐藏状态映射到词表大小的维度，得到每个 token 的预测分数 logits。训练时 logits 用于计算交叉熵损失，推理时 logits 用于选择下一个 token。

\subsection{模型配置与参数规模}

\subsubsection{核心超参数含义}

说明从零构建 GPT 时需要确定多个关键配置，包括词表大小、上下文长度、嵌入维度、注意力头数、Transformer 层数、dropout 比例、batch size 等。每个参数都会影响模型表达能力、训练速度和资源消耗。

建议加入一张表格：

配置项：vocab_size、context_length、emb_dim、n_heads、n_layers、dropout、batch_size
含义：词表大小、最大上下文长度、隐藏向量维度、注意力头数、Transformer 堆叠层数、随机失活比例、每次训练样本数
影响：参数量、显存/内存占用、训练速度、生成效果、过拟合风险

\subsubsection{参数规模与低算力适配}

说明模型参数量主要来自词嵌入层、位置嵌入层、注意力层、前馈网络和输出层。其中词表大小和嵌入维度会显著影响嵌入层与输出层参数量；层数和嵌入维度会影响 Transformer 主体参数量；上下文长度会显著影响注意力计算量。低算力条件下需要在模型表达能力与训练成本之间折中。

\subsubsection{小模型与真实大模型的关系}

说明本项目中的 MiniGPT 虽然参数量远小于真实大模型，但它保留了 GPT 类模型的核心结构和训练范式，因此能够用于验证从零构建语言模型的基本流程。真实大模型通常会使用更大规模语料、更大参数量、更长上下文、更复杂的分词器和分布式训练系统，而本项目重点在于理解原理和完成可运行的端到端实验。

\subsection{预训练方法与评价指标}

\subsubsection{预训练数据构造}

说明预训练样本通常通过滑动窗口从 token 序列中构造。每个样本包含固定长度的上下文，目标序列为输入序列右移一位。窗口长度对应模型的 context length，步长 stride 会影响样本数量和样本重叠程度。stride 较小样本更多、上下文覆盖更充分，但训练时间更长；stride 较大样本更少、训练速度更快，但可能降低数据利用率。

\subsubsection{预训练损失与优化器}

说明模型通过交叉熵损失学习 next-token prediction。训练时通常使用 AdamW 等优化器更新模型参数。学习率、batch size、训练轮数、dropout、梯度裁剪等都会影响训练稳定性和收敛效果。这里只讲知识点，不展开具体实验曲线。

\subsubsection{验证集与过拟合判断}

说明训练过程中需要划分训练集和验证集。训练损失反映模型对训练样本的拟合程度，验证损失反映模型对未见数据的泛化能力。如果训练损失持续下降而验证损失不再下降甚至上升，说明模型可能出现过拟合。早停机制可以在验证效果不再提升时停止训练，避免无效训练和过拟合。

\subsubsection{Perplexity 与生成样例}

说明 perplexity 是语言模型常用评价指标，可理解为模型对下一个 token 的平均不确定程度。perplexity 越低，通常说明模型预测越准确。但对于小模型实验，仅依靠数值指标不够，还需要结合生成样例观察语法连贯性、主题一致性和重复问题。

\subsection{模型微调与任务适配}

\subsubsection{预训练与微调的区别}

说明预训练主要学习通用语言规律，使用的是无标签文本；微调则是在预训练模型基础上，使用带标签或特定格式的数据继续训练，使模型适应具体任务。预训练关注“会不会建模语言”，微调关注“能不能完成指定任务”。

\subsubsection{监督数据构建}

说明微调需要构造输入和标签。例如文本分类任务中，每条样本包括文本内容和类别标签；问答任务中，每条样本包括问题、上下文和答案；指令微调中，每条样本通常包括 instruction、input 和 output。监督数据的格式会直接决定模型学习到的任务形式。

\subsubsection{分类微调结构}

说明 GPT 原本输出的是 token 级隐藏状态，如果用于文本分类，需要将序列表示转换为文本级表示。常见方法包括取最后一个有效 token 的隐藏状态、平均池化、最大池化等，再接入分类头输出类别 logits。分类任务一般使用交叉熵损失进行训练。

\subsubsection{微调策略与参数更新}

说明微调可以采用全参数微调、冻结部分参数、只训练分类头等策略。全参数微调适应能力强，但训练成本和过拟合风险较高；只训练分类头成本低，但任务适配能力有限。低算力项目中可以根据任务难度选择较轻量的微调策略。

\subsubsection{微调评价指标}

说明分类任务常用准确率、损失值、混淆矩阵等指标评价效果。如果类别不均衡，还可以使用 precision、recall、F1-score。对于本文的小型实验，重点展示模型经过微调后是否能根据输入文本输出合理类别。

\subsection{本章小结}

总结从零构建大语言模型需要掌握的知识链条：首先准备语料并完成分词编码，然后根据 token 序列构造训练样本；接着设计 Decoder-only GPT 模型，配置词嵌入、位置嵌入、因果自注意力、前馈网络和输出层；随后通过 next-token prediction 完成预训练，并使用验证损失和生成样例观察模型效果；最后通过监督数据和任务头完成微调，使模型适应具体下游任务。本文后续章节将在这些知识点基础上，完成 MiniGPT 的具体设计、预训练实验和微调实验。
