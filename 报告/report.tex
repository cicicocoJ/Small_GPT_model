...
\begin{enumerate}
  \item 数据准备与分词编码：选取 Hugging Face 平台上的 TinyStories 数据集作为预训练语料，对文本进行清洗和拼接，并采用基于正则表达式的 tokenizer 将文本转换为 token id 序列；
  \item 训练样本构造：利用滑动窗口方法将 token 序列切分为固定长度的输入序列，并将其右移一位作为预测目标，构成 next-token prediction 训练样本；
  \item 小型 GPT 模型搭建：基于 PyTorch 构建小参数量 GPT 类模型，主要包括 token embedding、position embedding、因果多头自注意力、前馈网络、残差连接和层归一化等模块；
...
\textbf{自回归建模思想。} GPT 类语言模型的核心任务是根据已有上下文预测下一个 token。
文本经过分词后可表示为 token 序列 $x_1,x_2,\cdots,x_T$，模型并不是一次性生成完整文
本，而是按照从左到右的顺序逐步建模当前位置的条件概率。对应到训练阶段，next-token
prediction 要求模型根据前 $t$ 个 token 预测第 $t+1$ 个 token，其语言建模目标可写为：
\begin{equation}
\begin{aligned}
P(x_1,x_2,\cdots,x_T)
...
\textbf{Decoder-only 结构。} 为了服务于从左到右的生成目标，GPT 采用 Decoder-only
Transformer 结构，并通过因果遮罩限制每个位置只能使用当前位置及其之前的信息。这样，
模型在训练时不会提前看到未来 token，在推理时也可以不断把新生成的 token 拼接回上下文，
继续预测后续内容。本文实现的 MiniGPT 是该结构的小型化版本，保留 token embedding、
position embedding、因果自注意力、前馈网络、残差连接和 LayerNorm 等关键组件，但通过
较小的层数、嵌入维度和上下文长度降低训练成本。图~\ref{fig:llm_overall_structure} 给出了
大语言模型从文本输入到模型输出的整体结构关系，可作为后续 MiniGPT 实现流程的概念参考。
...
\subsection{文本分词与输入表示}

\textbf{Tokenizer 的作用。} 神经网络不能直接处理原始字符串，因此文本需要先经过 tokenizer
转换为 token，再映射为整数形式的 token id。Tokenizer 主要完成文本切分、词表构建、编
码和解码：编码将文本转为模型输入，解码则将模型生成的 token id 还原为可读文本。本项
目保留正则 tokenizer 和 GPT-2 BPE tokenizer 两种方案。正则 tokenizer 词表较小，更适合
小模型和低算力训练；GPT-2 BPE tokenizer 更接近真实 GPT 系列模型常用的子词分词方式，
但固定词表较大，会增加输出层参数量。预训练语料中使用 \texttt{<|endoftext|>} 作为故事边
界标记，具体数据拼接、缓存和样本构造过程放在第四章说明。

\textbf{输入表示。} 得到 token id 后，模型还需要将离散编号映射为连续向量。GPT 的输入
表示由 token embedding 和 position embedding 相加得到：
\begin{equation}
\mathbf{h}_0 = \mathbf{E}_{\mathrm{tok}}(\mathbf{x}) + \mathbf{E}_{\mathrm{pos}}(\mathbf{p}) .
\label{eq:gpt_input_embedding}
\end{equation}
其中，$\mathbf{x}$ 表示 token id 序列，$\mathbf{p}$ 表示位置编号序列。Token embedding
提供 token 本身的表示，position embedding 提供顺序信息，二者共同构成 Transformer Block
的输入。

\subsection{Transformer 核心机制}

\textbf{Transformer Block 的作用。} GPT 的主体由多层 Transformer Block 堆叠而成。
Transformer Block 通过因果自注意力建模上下文关系，再通过前馈网络进行非线性变换，并
使用残差连接和 LayerNorm 稳定训练。对本文项目而言，需要重点理解的是：自注意力让模型
能够从上下文中选择相关信息，因果遮罩保证模型不能看到未来 token，多层堆叠则逐步形成
更高层次的文本表示。图~\ref{fig:transformer_core_structure} 展示了 Transformer 模块中
注意力、前馈网络以及残差归一化结构之间的关系。
...
\textbf{因果自注意力机制。} 注意力计算通常由 Query、Key 和 Value 三组向量完成。在 GPT
中，为了满足自回归生成约束，需要在注意力分数中加入因果遮罩矩阵 $\mathbf{M}$：
\begin{equation}
...
\end{equation}
其中，$\mathbf{M}$ 表示因果遮罩矩阵，未来位置对应的注意力分数被置为负无穷，从而避免
模型看到未来 token。多头注意力则是在多个子空间中并行执行上述注意力计算，使模型能够
从不同角度捕捉上下文关系。

\subsection{预训练与微调机制}

\textbf{预训练。} 预训练使用无标签文本作为数据来源，通过 next-token prediction 让模型
学习通用语言模式。本项目的预训练阶段对应 TinyStories 语料，目标是让小型 MiniGPT 在有
限数据和算力条件下学习基本英文短故事中的局部上下文关系。预训练结果主要通过训练损失、
验证损失、perplexity 以及生成样例进行观察，具体实验设置和结果放在第四章展开。

\textbf{微调。} 微调是在预训练模型基础上，使用带标签或具有明确任务格式的数据继续训练，
使模型适应具体任务。与预训练关注通用语言规律不同，微调更关注任务目标，例如文本分类
或问答。本文后续微调实验会在预训练 MiniGPT 的基础上构建简单监督任务，相关分类头设计、
损失函数和评价指标放在第五章说明。
...
从整体结构看，\texttt{MiniGPT} 包含 token embedding、position embedding、embedding
Dropout、多层 \texttt{TransformerBlock}、final LayerNorm 以及无偏置线性输出层。该结构
...
为连续向量表示，并与位置编码相加；随后隐藏状态经过多层因果 Transformer Block 建模上下
文关系；最后通过 final LayerNorm 和线性输出层得到词表维度上的 logits。后续预训练、文本
生成和微调实验均建立在这一模型主体之上。
...
嵌入维度、注意力头数、Transformer Block 层数、正则化强度以及 Query、Key、Value 线性
映射是否使用偏置。
...
\subsubsection{前馈网络与 Transformer Block}

\texttt{FeedForward} 模块采用“两层线性层 + GELU + Dropout”的结构。第一层线性层将隐
藏维度从 \texttt{emb\_dim} 扩展到 \texttt{4 * emb\_dim}，经过 GELU 非线性激活后，再由第
二层线性层映射回 \texttt{emb\_dim}。这种先升维再降维的设计是 GPT 类 Transformer Block
中的常见结构，用于增强每个位置隐藏表示的非线性表达能力。

\texttt{TransformerBlock} 由因果自注意力子层和前馈网络子层组成，并采用 Pre-LayerNorm
...
\texttt{MiniGPT} 是整个模型的主体类，内部定义了 token embedding、position embedding、
embedding Dropout、多个 \texttt{TransformerBlock}、final LayerNorm 和无偏置线性输出层。
在 forward 过程中，模型首先检查输入序列长度是否超过 \texttt{context\_length}；随后根据
当前序列长度自动生成 position id，并分别计算 token embedding 和 position embedding。
二者相加后经过 Dropout，再依次输入 Transformer blocks、final LayerNorm 和线性输出层。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lll}
    ...
    \texttt{n\_layers} & Transformer Block 层数 & $2$ \\
...
大语言模型，该规模非常小，但已经足以展示 GPT 类模型中的 token embedding、位置嵌入、
...
综上，本章完成了小型 GPT 模型架构的搭建。该模型保留了 token embedding、position
embedding、因果多头自注意力、前馈网络、残差连接和 LayerNorm 等核心机制，同时通过较
...
\texttt{train[:5000]} 和验证集切片 \texttt{validation[:500]}，读取字段为 \texttt{text}。
这种规模既能提供较为丰富的短故事文本，又不会使 CPU 单机训练时间过长，符合低算力课程
实验的目标。

\textbf{文本拼接与清洗。} 程序读取每条故事文本后，将多条故事拼接为连续语料，并在不同
故事之间加入 \texttt{<|endoftext|>} 作为分隔标记，用于表示故事边界。文本清洗主要包括
统一 Windows 和 Linux 换行、去除行尾多余空格、压缩连续空行等基础操作。为了便于重复实
验，程序设置了 Hugging Face 数据缓存目录，但报告正文不展开具体本地路径，相关文件统一
作为实验输出管理。
...
\textbf{Tokenizer 选择。} 本项目的 \texttt{tiny\_tokenizer.py} 中保留了两种 tokenizer：
\texttt{GPT2Tokenizer} 和 \texttt{SimpleRegexTokenizer}。前者使用 GPT-2 BPE tokenizer，
文本适应能力更强，但固定词表较大，会显著增加词嵌入层和输出层参数量；后者是本实验采
用的主方案，它将 \texttt{<|endoftext|>} 作为完整特殊 token，同时将英文单词、数字、标点
和换行符等切分为 token。与字符级分词相比，正则分词更容易保留完整单词；与 GPT-2 BPE
相比，正则分词词表更小，更适合低算力小模型实验。本次预训练中，正则 tokenizer 由训练
语料构建词表，最终词表大小为 $8785$。
...
预训练任务采用 next-token prediction。MiniGPT 对每个 batch 输出形状为
$[\mathrm{batch\_size},\mathrm{seq\_len},\mathrm{vocab\_size}]$ 的 logits，程序将 logits 与
...
数据读取后，程序复用预训练阶段保存的 tokenizer 对文本进行编码，并根据模型上下文长度进行截断和 padding，同时生成 \texttt{attention\_mask} 用于区分真实 token 与 padding token。数据集采用固定随机种子进行分层划分，训练集、验证集和测试集数量分别为 210、45 和 45。由于测试集中正负类别样本数量并非完全相同，除 accuracy 外，本文还报告 precision、recall、F1、macro F1 和 weighted F1，以更全面地衡量模型在不同类别上的表现。表~\ref{tab:finetune_dataset} 给出了微调数据集划分与标签设置。
...
\begin{table}[H]
  ...
  \begin{tabular}{L{3.2cm}L{3.2cm}L{3.0cm}L{5.0cm}}
    ...
    标签 0 & \texttt{negative} & -- & 结局消极、问题未解决、人物受伤或难过 \\
    标签 1 & \texttt{positive} & -- & 结局积极、问题解决、人物安全或获得帮助 \\
...
微调模型在预训练 MiniGPT 主体后增加一个线性分类头，而不改变原有 GPT 主体结构。输入文本经过 token embedding、position embedding、Transformer blocks 和 final norm 后，得到每个 token 的隐藏状态。分类任务需要整段文本的表示，因此本文采用 mean pooling 将所有有效 token 的隐藏状态合成为文本级向量，再送入线性分类头，输出 \texttt{negative} 和 \texttt{positive} 两个类别的 logits。训练目标使用交叉熵损失。

在初始化方式上，微调程序加载预训练阶段验证集表现最优的 MiniGPT 参数，并复用预训练阶段的 tokenizer 和词表，从而保证预训练与微调阶段的 token 编码方式一致。参数更新策略上，本文最终采用全参数微调，即 MiniGPT 主体与分类头全部参与更新。这样做的原因是本项目中的 MiniGPT 规模较小，仅包含 2 层 Transformer block，若只训练分类头或最后一层，模型对下游分类任务的适配能力会受到限制。
...
本章围绕故事结局倾向二分类任务完成了监督微调实验。实验构建了带标签的故事结尾数据，确定了全参数微调与 mean pooling 的分类策略，并在预训练 MiniGPT 基础上实现了分类微调程序。实验通过 accuracy、F1、混淆矩阵和测试样例展示了模型处理特定任务的效果，测试集准确率达到 84.44\%。同时，结果也表明小模型和小规模数据仍存在局限：模型对 positive 类识别更稳定，对包含负面事件但带有积极措辞的边界样本仍容易误判，后续可通过补充负向样本和改进标注一致性进一步提升泛化能力。

\subsection{本章小结}

本章完成了故事结局倾向二分类任务的监督微调实验。实验构建了带标签的故事结尾数据，并在预训练 MiniGPT 基础上加入分类头，使模型能够从文本生成任务适配到具体分类任务。结果表明，微调后的模型能够较好地区分积极和消极结局，测试集准确率达到 84.44%。同时，实验也说明小模型在边界样本判断上仍存在不足，后续可通过扩充数据、改进标注和优化微调策略进一步提升效果。

\section{总结与体会}

\textbf{一、成果评价。}

本项目围绕“从零构建大模型”的课程要求，完成了从理论学习到实验实现的完整流程。项目首先梳理了 GPT 类语言模型、Tokenizer、Transformer、预训练与微调等关键知识点；随后基于 PyTorch 搭建了小参数量 MiniGPT 模型，并在 TinyStories 数据集上完成预训练和文本生成测试；最后，通过故事结局倾向二分类任务完成了监督微调实验。整体来看，本项目虽然没有训练真实的大规模商业模型，但在低算力条件下实现了一个可训练、可推理、可微调的小型 GPT 类模型，基本覆盖了课程任务中关于模型搭建、预训练、微调和结果分析的要求。
...
\item regex tokenizer 泛化能力有限，不如成熟的 BPE tokenizer；
...
通过本次项目，我对大语言模型的底层流程有了更完整的认识。过去使用大模型时，我更多关注的是输入和输出效果，而这次从文本分词、token 编码、词嵌入、位置嵌入，到 Transformer Block、loss 计算和自回归生成，逐步理解了 GPT 模型内部的基本运行方式。尤其是因果注意力机制让我更清楚地认识到，GPT 的生成过程本质上是在已有上下文条件下不断预测下一个 token。
...
\begin{enumerate}
  ...
  \item tiktoken 分词工具相关资料。本文在分词方案设计中保留 GPT-2 BPE tokenizer 作为可选方案，相关实现思想参考了 tiktoken 工具说明。
\end{enumerate}
...
\begin{itemize}
  \item \texttt{vocab.json}：tokenizer 词表与配置；
