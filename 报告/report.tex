...
\begin{quote}
输入 token ids $\rightarrow$ Token Embedding + Position Embedding $\rightarrow$
Transformer Block $\times N$ $\rightarrow$ Final LayerNorm $\rightarrow$ Linear Head
$\rightarrow$ logits
\end{quote}

上述流程体现了小型 GPT 的基本计算路径：离散 token id 首先被映射为连续向量表示，再经
过多层因果 Transformer Block 建模上下文关系，最终输出词表维度上的 logits。后续预训练、
文本生成和微调实验均建立在这一模型主体之上。

% 后续可加入图：pic/MiniGPT模型结构图.png
